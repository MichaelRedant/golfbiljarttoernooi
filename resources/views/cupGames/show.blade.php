@extends('layouts.app')

@section('content')
<div class="container">
    <h3 class="text-center">{{ $game->round->round_name }} Wedstrijddetails</h3>

    <div class="row">
        <div class="col-md-6">
            <h4 class="text-center">{{ $game->homeTeam->name }}</h4>
            <p class="text-center">
                <a href="{{ route('teams.show', $game->homeTeam->id) }}">
                    {{ $game->homeTeam->name }}
                </a>
            </p>
        </div>
        <div class="col-md-6">
            <h4 class="text-center">{{ $game->awayTeam->name }}</h4>
            <p class="text-center">
                <a href="{{ route('teams.show', $game->awayTeam->id) }}">
                    {{ $game->awayTeam->name }}
                </a>
            </p>
        </div>
    </div>

    <hr>

    <div class="row">
        <div class="col-md-6 text-center">
            <strong>Locatie:</strong>
            <p>{{ $game->homeTeam->location }}</p>
        </div>
        <div class="col-md-6 text-center">
            <strong>Datum:</strong>
            <p>{{ \Carbon\Carbon::parse($game->date)->format('d-m-Y') }}</p>
        </div>
    </div>

    <div class="row">
        <div class="col-md-12 text-center">
            @if($game->return_game_id)
                <h5 class="mb-3">
                    <strong>Score van deze wedstrijd:</strong> 
                    <span class="badge">{{ $game->home_score ?? 'N/A' }}</span> - 
                    <span class="badge">{{ $game->away_score ?? 'N/A' }}</span>
                </h5>
                <h5 class="mb-3">
                    <strong>Totale score van heen- en terugwedstrijd:</strong>
                    <span class="badge">{{ $totalScore['total_home_score'] ?? $game->home_score }}</span> - 
                    <span class="badge">{{ $totalScore['total_away_score'] ?? $game->away_score }}</span>
                </h5>
            @else
                <h5 class="mb-3">
                    <strong>Wedstrijdscore:</strong>
                    <span class="badge">{{ $game->home_score ?? 'N/A' }}</span> - 
                    <span class="badge">{{ $game->away_score ?? 'N/A' }}</span>
                </h5>
            @endif
        </div>
    </div>

    <div class="row">
        <div class="col-md-6 text-center">
            <strong>Kapitein:</strong>
            <p><a href="#">{{ $liveData['home_captain_name'] ?? $game->homeTeam->captain->full_name ?? 'Onbekend' }}</a></p>
        </div>
        <div class="col-md-6 text-center">
            <strong>Kapitein:</strong>
            <p><a href="#">{{ $liveData['away_captain_name'] ?? $game->awayTeam->captain->full_name ?? 'Onbekend' }}</a></p>
        </div>
    </div>

    <div class="row">
        <div class="col-md-6 text-center">
            <strong>Reservespeler:</strong>
            <p><a href="#">{{ $liveData['home_reserve_name'] ?? $game->homeTeam->reserve->full_name ?? 'Onbekend' }}</a></p>
        </div>
        <div class="col-md-6 text-center">
            <strong>Reservespeler:</strong>
            <p><a href="#">{{ $liveData['away_reserve_name'] ?? $game->awayTeam->reserve->full_name ?? 'Onbekend' }}</a></p>
        </div>
    </div>

    @if(!empty($scores))
    <div class="mt-4">
        <h4 class="text-center">Scores</h4>
        <table class="table table-striped">
            <thead>
                <tr class="text-center">
                    <th>Speler {{ $game->homeTeam->name }}</th>
                    <th>Team Thuis</th>
                    <th>Speler {{ $game->awayTeam->name }}</th>
                    <th>Team Uit</th>
                    <th>1M</th>
                    <th>2M</th>
                    <th>Belle</th>
                </tr>
            </thead>
            <tbody>
                @foreach($scores as $score)
                    <tr class="text-center">
                        <td>{{ $score['home_player_name'] }}</td>
                        <td>{{ $score['home_player_team'] }}</td>
                        <td>{{ $score['away_player_name'] }}</td>
                        <td>{{ $score['away_player_team'] }}</td>
                        <td>{{ $score['1M'] }}</td>
                        <td>{{ $score['2M'] }}</td>
                        <td>{{ $score['Belle'] }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    </div>
    @endif

    @if(!empty($liveData['testmatch_scores']))
    <div class="mt-4">
        <h4 class="text-center">Testmatch Scores</h4>
        <table class="table table-striped">
            <thead>
                <tr class="text-center">
                    <th>Speler {{ $game->homeTeam->name }}</th>
                    <th>Speler {{ $game->awayTeam->name }}</th>
                    <th>1M</th>
                </tr>
            </thead>
            <tbody>
                @foreach($liveData['testmatch_scores'] as $testmatch)
                    <tr class="text-center">
                        <td>{{ $testmatch['home_player_name'] }}</td>
                        <td>{{ $testmatch['away_player_name'] }}</td>
                        <td>{{ $testmatch['1M'] }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    </div>
    @endif

    <div class="text-center mt-4">
        @if($cup->division)
            <a href="{{ route('cups.show', ['cup' => $cup->id]) }}" class="btn btn-primary">Terug naar Cup</a>
        @else
            <a href="#" onclick="history.back()" class="btn btn-primary">Terug</a>
        @endif
    </div>

    @if(auth()->check() && auth()->user()->isAdmin() && !$game->away_team_approved)
    <form action="{{ route('cupGames.approve', ['cup' => $cup->id, 'game' => $game->id]) }}" method="POST" class="text-center mt-4">
        @csrf
        <button type="submit" class="btn btn-success">Goedkeuren</button>
    </form>
    @endif
</div>
@endsection
